...
class TokenManager:
    ...
    def get_websocket_approval_key(self):
        """WebSocket 접속을 위한 approval key 발급 - 수정된 버전"""
        try:
            # 매번 새로운 승인키 발급 (이전에 성공한 방식 사용)
            ...
            body = {
                "grant_type": "client_credentials",
                "appkey": self.config['api_key'],
                "secretkey": self.config['api_secret']  # 'appsecret'이 아닌 'secretkey' 사용
            }
            
            # 헤더에는 Authorization 불필요 (테스트에서 성공한 방식)
            headers = {"Content-Type": "application/json"}
